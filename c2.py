...
def build_variant_to_build_dir(build_variant):
    ...
    if build_variant.cxxstd is not None:
        build_dir += f"_{build_variant.cxxstd}"
    ...
    return os.path.join("build_c2y", build_dir)
...
def build_variant_to_cmake_configure_args(build_variant: BuildVariant, build_dir: str):
    ...
    config_args = [
        ...
        "-B", build_dir,
        "-DBUILD_TESTING=ON",
        "-DBOOST_INCLUDE_LIBRARIES=hash2",
...
def parse_args():
    ...
    if args.cxxstd:
        ...
        print("Invalid input. Make sure to use the form `--cxxstd 11,20`.")

    if args.toolset:
        result = args.toolset.split(",")
        toolsets += result
    else:
        print("Invalid input. Make sure to use the form `--toolset gcc-14,clang-19`.")

    if args.variant:
        result = args.variant.split(",")
        variants += result
    else:
        print("Invalid input. Make sure to use the form `--variant debug,release`.")

    if args.address_model:
        result = args.address_model.split(",")
        address_models += result
    else:
        print("Invalid input. Make sure to use the form `--address-model 32,64`.")
